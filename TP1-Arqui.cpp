...
int main(int argc,char **argv)
{
    ...
    if(numprocs == 1)
    {
        parcial_de_M_para_B = new int[n*n];
        sendcounts_B[0]= n * n;
    }
    else
    {
        ...
        for (int i=0; i<n * n/numprocs; i++)
        {
            ...
            if(medio)
            {
                if(i >= (n*n)-n)//ultima fila
                {
                    if(i%n == 0)
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i+1];
                    }
                    else if(i%n == (n-1))
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i-1];
                    ...
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i-1] + parcial_de_M_para_B[i+1];
                    }
                }
                else//medio
                {
                    if(i%n == 0)
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i+1];
                    }
                    else if(i%n == (n-1))
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i-1];
                    ...
                    {
                        B_parcial[i] = parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i-1] + parcial_de_M_para_B[i+1];
                    }
                }
            }
            else//primera
            {
                if(i%n == 0)
                {
                    B_parcial[i] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i+1] +parcial_de_M_para_B[n];
                }
                else if(i%n == (n-1))
                {
                    B_parcial[i] = parcial_de_M_para_B[i]+ parcial_de_M_para_B[i-1] + parcial_de_M_para_B[n+i];
                }
                else
                {
                    B_parcial[i] = parcial_de_M_para_B[i-1] + parcial_de_M_para_B[i] + parcial_de_M_para_B[i+1] + parcial_de_M_para_B[i+n];
                }
            }
        }
    }
    ...
    {
        for (int i=n; (i-n)<n * n/numprocs; i++)//medio
        {
            if(i%n == 0)
            {
                B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i+1];
            }
            else if(i%n == (n-1))
            {
                B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i-1];
            ...
            {
                B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i+n] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i-1] + parcial_de_M_para_B[i+1];
            }
        }
    }
    else
    {
        ...
        for (int i=n; (i-n)<n * n/numprocs; i++)
        {
            if (i%n==0 && i!=n)
            ...
            if(cantidadFilasRestantes == 1)//ultima fila
            {
                if(i%n == 0)
                {
                    B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i+1];
                }
                else if(i%n == (n-1))
                {
                    B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i-1];
                ...
                {
                    B_parcial[i-n] = parcial_de_M_para_B[i] + parcial_de_M_para_B[i-n] + parcial_de_M_para_B[i-1] + parcial_de_M_para_B[i+1];
                }
            }
            else//medio
            {
